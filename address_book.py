...
class AddressBookApp(tk.Frame):
    ...
    def _create_widgets(self):
        tree_view_entries = ["Entry 1", "Entry 2", "Entry 3", "Entry 4"]
    
        treeview_widgets = ttk.Treeview(self.master)
        index = 1
        for entry in tree_view_entries:
            treeview_widgets.insert("","end",index,text=entry)
            index += 1
        treeview_widgets.pack()
        self._create_menus()
